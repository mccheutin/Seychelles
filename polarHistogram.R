...
polarHistogram<-function(
  ...
  # histograms
  p<-ggplot(df)+geom_rect(aes(xmin=xmin,xmax=xmax,ymin=ymin,ymax=ymax,fill=score))+labs(fill="Score")
  ...
  p<-p+geom_text(
    aes(
      ...
      hjust=hjust),
    y=1.02,
    size=1.5,# to modify the size of the samples
    vjust=0.5,
    data=dfItemLabels)
...
p<-p+geom_text(aes(x=x,label=family,angle=angle),data=familyLabelsDF,y=1.35,color="black",fontface = "bold",size=8, family = "serif")
  } 
